...
def live_search(request):
    q = request.GET.get("q", "")
    information = Product.objects.filter(Q(sku__icontains=q) | Q(name__icontains=q) | Q(description__icontains=q))
    information = serializers.serialize('json', information)
    return JsonResponse({'information': information})